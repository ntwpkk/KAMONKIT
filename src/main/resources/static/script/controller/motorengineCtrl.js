...
angular
    .module('app')
        .controller('motorengineCtrl', [ '$scope', '$http','currentAuth' , 'Auth' ,function($scope,$http,currentAuth,Auth) {
        	...
        	$scope.fillInData = function(){
	    		...
	    		$scope.motorcycle.engineNumber = null;
        	}
        	
        	$scope.updateChassis = function(){
        		console.log($scope.motorcycle);
        		$http.post('/motorcycle/updateEngine?updatedBy='+currentAuth.uid, $scope.motorcycle)
		    	.then(function(response){
		    		$scope.beforeSearch = true;
		    		new PNotify({
                        title: 'Success',
