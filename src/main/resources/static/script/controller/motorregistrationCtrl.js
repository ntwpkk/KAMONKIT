...
angular
    .module('app')
        .controller('motorregistrationCtrl', [ '$scope', '$http','currentAuth' , 'Auth' ,function($scope,$http,currentAuth,Auth) {
        	...
        	$scope.fillInData = function(){
	    		...
	    		$scope.motorcycle.registrationNumber = null;
	    		$scope.motorcycle.registrationNumber = null;
	    		$scope.registrationNumber = $scope.registNumber+' '+$scope.registProvince;
        	}
        	
        	$scope.updateRegistration = function(){
        		console.log($scope.motorcycle);
        		$http.post('/motorcycle/updateRegistration?updatedBy='+currentAuth.uid, $scope.motorcycle)
		    	.then(function(response){
		    		$scope.beforeSearch = true;
		    		new PNotify({
                        title: 'Success',
